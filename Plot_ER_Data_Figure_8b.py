...
def PlotPatchBins(Sc,PatchData,NumBins,color,MinimumBinSize=10,ErrorBars=True):
    """
    ...
    if ErrorBars:
        ...
        if MinimumBinSize == 1: plt.errorbar(bin_x, bin_y, fmt='o',color=color,label='Min. Bin Size = 0.1%')
        if MinimumBinSize == 7: plt.errorbar(bin_x, bin_y, fmt='o',color=color,label='Min. Bin Size = 1%')
        if MinimumBinSize == 68: plt.errorbar(bin_x, bin_y, fmt='o',color=color,label='Min. Bin Size = 10%')
        if MinimumBinSize == 101: plt.errorbar(bin_x, bin_y, fmt='o',color=color,label='Min. Bin Size = 15%')
...
def MakeThePlot(q,Path,Prefix,Sc_Method,RawFlag,DensityFlag,BinFlag,NumBins,PatchFlag,BasinFlag,LandscapeFlag,Order,ErrorBarFlag=True,Format='png',ComparisonData=(False,False,False),NumBootsraps=10000):
    """
    ...
    if BinFlag == 'patches':
        if q == 1:
            DrawCurve()
            PlotPatchBins(Sc,PatchData,NumBins,'gold',MinimumBinSize=q,ErrorBars=ErrorBarFlag)
        if q == 7: PlotPatchBins(Sc,PatchData,NumBins,'darkorange',MinimumBinSize=q,ErrorBars=ErrorBarFlag)
        if q == 68: PlotPatchBins(Sc,PatchData,NumBins,'r',MinimumBinSize=q,ErrorBars=ErrorBarFlag)
        if q == 101: PlotPatchBins(Sc,PatchData,NumBins,'darkred',MinimumBinSize=q,ErrorBars=ErrorBarFlag)
