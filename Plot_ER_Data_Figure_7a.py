...
def PropagateErrors(PatchData,BasinData):
    ...
    basinLH = unp.uarray(BasinData[5],BasinData[9])
    basinR = unp.uarray(BasinData[7],BasinData[9])
    basinCHT = unp.uarray(BasinData[6],BasinData[9])
...
def MakeThePlot(Path,Prefix,Sc_Method,RawFlag,DensityFlag,BinFlag,NumBins,PatchFlag,BasinFlag,LandscapeFlag,Order,ErrorBarFlag=True,Format='png',ComparisonData=(False,False,False),NumBootsraps=10000):
    ...
    if PatchFlag:
        #PlotPatches(Sc,PatchDataErrs,ErrorBarFlag) (0,0,0.)
        PlotPatchesArea(Sc,PatchData,0,'gold')
        PlotPatchesArea(Sc,PatchData,50,'darkorange')
        PlotPatchesArea(Sc,PatchData,100,'r')
        PlotPatchesArea(Sc,PatchData,500,'darkred')
    if BinFlag == 'patches':
