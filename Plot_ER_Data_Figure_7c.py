...
def PlotPatchesArea(Sc,PatchData,thresh,alpha):
    ...
    #plt.scatter(x,y,c='blue',edgecolor='',label='Min. Patch Area = '+str(thresh))
    
    #plt.plot(x,y,color='b',alpha=alpha,marker='o',linestyle='',markeredgecolor=(0,0,1,alpha),label='Min. Patch Area = '+str(thresh))   
    plt.plot(x,y,color=alpha,marker='o',linestyle='',markeredgecolor=alpha,label='Min. Patch Area = '+str(thresh))   
...
def PlotBasinsArea(Sc,BasinData,order,color):
    """
    ...
    plt.plot(e_star,r_star,color=color,marker='o',linestyle='',markeredgecolor=color,label='Basin Order = '+str(order))   
...
def SavePlot(Path,Prefix,Format):
    ...
    file format.
    """
    plt.savefig(Path+'c'+Prefix+'_E_R_Star.'+Format,dpi=500)
    plt.clf()
...
def MakeThePlot(Path,Prefix,Sc_Method,RawFlag,DensityFlag,BinFlag,NumBins,PatchFlag,BasinFlag,LandscapeFlag,Order,ErrorBarFlag=True,Format='png',ComparisonData=(False,False,False),NumBootsraps=10000):
    ...
    if BasinFlag:
        ...
        if Order==1: 
            DrawCurve()
            PlotBasinsArea(Sc,BasinData,Order,'gold')
        if Order==2: PlotBasinsArea(Sc,BasinData,Order,'darkorange')
        if Order==3: PlotBasinsArea(Sc,BasinData,Order,'r')
        if Order==4: PlotBasinsArea(Sc,BasinData,Order,'darkred')
